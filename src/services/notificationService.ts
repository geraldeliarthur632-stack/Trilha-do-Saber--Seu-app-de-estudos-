...
class NotificationService {
  ...
  // Check regular study reminders (including advance FCM reminders)
  private checkAndTriggerReminders() {
    ...
    for (const rem of this.reminders) {
      ...
      if (!rem.daysOfWeek.includes(currentDay)) continue;

      const [rh, rm] = (rem.time || '14:00').split(':').map(Number);
      if (isNaN(rh) || isNaN(rm)) continue;
      const targetTotalMinutes = rh * 60 + rm;

      // 1. Advance notification (when the schedule is close to starting a specific subject)
      const advanceMinutes =
        rem.advanceNoticeMinutes !== undefined
          ? rem.advanceNoticeMinutes
          : fcmReminderService.getAdvanceNoticeMinutes();

      if (advanceMinutes > 0 && currentTotalMinutes === targetTotalMinutes - advanceMinutes) {
        const advanceKey = `fcm_advance_${rem.id}_${todayDateStr}_${advanceMinutes}`;
        if (!this.lastTriggered[advanceKey]) {
          this.lastTriggered[advanceKey] = true;
          this.triggerAdvanceFCMReminder(rem, advanceMinutes);
        }
      }
  ...
  // Trigger Advance FCM Reminder (Notification sent before subject starts)
  public triggerAdvanceFCMReminder(reminder: StudyReminder, advanceMinutes: number, isTest = false) {
    const studentName = typeof window !== 'undefined' ? localStorage.getItem('estudahud_student_name') || 'Estudante' : 'Estudante';
    const personalized = fcmReminderService.generatePersonalizedReminder({
      studentName,
    ...
    const notifOptions: any = {
      ...
      icon: '/icon.svg',
      badge: '/icon.svg',
      tag: `fcm_advance_${reminder.id}_${Date.now()}`,
      requireInteraction: true,
      vibrate: [300, 100, 300, 100, 300],
      data: { url: `/?mode=journey&subject=${reminder.subjectId}`, reminderId: reminder.id, type: 'fcm_advance' },
      actions: [
    ...
    // Native Notification / Service Worker Push
    if (typeof window !== 'undefined' && 'serviceWorker' in navigator && 'Notification' in window && Notification.permission === 'granted') {
      navigator.serviceWorker.ready
        .then((reg) => {
          reg.showNotification(personalized.title, notifOptions);
        })
        .catch(() => {
          try {
            new Notification(personalized.title, notifOptions);
          } catch {}
        });
    } else if (typeof window !== 'undefined' && 'Notification' in window && Notification.permission === 'granted') {
      try {
    ...
    if (reminder.soundAlert !== false && this.isReminderSoundEnabled()) {
      try {
        soundEffects.playStudyReminderChime();
      } catch {}
    }
    ...
    if (reminder.voiceAlert !== false) {
      speechNarrator.speak(
        `Atenção ${studentName}! O cronograma indica que sua matéria de ${reminder.subjectName} começará em ${advanceMinutes} minutos. Prepare seu material para iniciar com foco!`
      );
    }
    ...
    try {
      this.recordReminderHistory({
        ...
        actionNote: `Aviso prévio FCM: ${advanceMinutes} min antes do início de ${reminder.subjectName}`,
      });
    } catch {}
