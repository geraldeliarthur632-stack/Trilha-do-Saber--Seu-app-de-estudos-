...
type JourneyStep = 'subject_select' | 'topic_select' | 'lesson_intro' | 'quiz' | 'summary';
...
export const JourneyMode: React.FC<JourneyModeProps> = ({
  ...
  const [currentStep, setCurrentStep] = useState<JourneyStep>(
    initialSubjectId ? 'topic_select' : 'subject_select'
  );
  ...
  // Navigate to Content/Topic Selection
  const handleProceedToTopicSelect = () => {
  ...
  // Launch a selected lesson (from specific topic or default)
  const handleSelectLesson = (lesson: LessonPlan, autoNarrate = true) => {
    soundEffects.playClick();
    speechNarrator.stop();
  ...
  // Seguir Padrão (Recomendado pelo App)
  const handleFollowDefaultTopic = () => {
    handleSelectLesson(defaultTopic.lesson, true);
  };

  // Start Journey - redirects to topic select or follows default
  const handleStartJourney = () => {
    handleProceedToTopicSelect();
  };
    ...
    <div className="flex-1 flex flex-col p-4 bg-[#070a12] max-w-lg mx-auto w-full pb-20 select-none">
      ...
      <div className="flex items-center justify-between mb-4">
        <button
          ...
          className="p-2.5 rounded-2xl bg-[#121829] hover:bg-[#161f38] text-slate-300 hover:text-white border border-[#1e293b] transition cursor-pointer"
        >
        ...
        {currentStep === 'topic_select' && (
          <div className="text-center">
            <h1 className="text-sm font-black text-white">Conteúdos do Ano</h1>
        ...
        {currentStep === 'lesson_intro' && (
          <div className="text-center">
            ...
            <span className="text-[10px] text-[#c084fc] font-bold truncate max-w-[200px] block">
      ...
      {!isLoading && currentStep === 'subject_select' && (() => {
        ...
        return (
          <div className="flex-1 flex flex-col justify-between space-y-4">
            ...
            {/* Action Buttons: Escolher Conteúdo ou Seguir Padrão Direto */}
            <div className="pt-2 space-y-2.5">
              <button
                onClick={handleProceedToTopicSelect}
              ...
              >
                <span>Continuar para Conteúdos do Ano</span>
                <ChevronRight className="w-4 h-4" />
              </button>

              <button
                onClick={handleFollowDefaultTopic}
                className="w-full py-2.5 px-4 rounded-xl bg-[#121829] hover:bg-[#161f38] text-slate-300 hover:text-white border border-[#1e293b] font-bold text-xs transition flex items-center justify-center gap-2 cursor-pointer active:scale-98"
              >
                <Sparkles className="w-3.5 h-3.5 text-amber-400" />
                <span>Ou Seguir Padrão do App Direto</span>
              </button>
            </div>
          </div>
        );
      ...
      {/* STEP 1.5: TOPIC SELECT - ESCOLHER CONTEÚDO DO ANO OU SEGUIR PADRÃO */}
      {!isLoading && currentStep === 'topic_select' && (
        <div className="flex-1 flex flex-col justify-between space-y-4 overflow-hidden">
          <div className="space-y-4 overflow-y-auto pr-1 flex-1">
            {/* Subject Banner */}
            <div className="p-4 rounded-3xl bg-[#121829] border border-[#1e293b] flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <div
                  className={`w-11 h-11 rounded-2xl ${currentSubjectMeta.iconBg} flex items-center justify-center text-xl shadow-md shrink-0`}
                >
                  {currentSubjectMeta.icon}
                </div>
                <div>
                  ...
                  <span className="text-xs text-slate-400 font-semibold">
                    Conteúdos do {GRADE_LABELS[user.grade]?.short || 'Ano Escolar'}
                  </span>
                </div>
              </div>
              <button
                ...
                className="text-xs text-slate-400 hover:text-[#c084fc] font-bold underline cursor-pointer"
              >
                Trocar Matéria
              </button>
            </div>

            {/* HERO CARD: SEGUIR PADRÃO (RECOMENDADO PELO APP) */}
            <div className="p-4 rounded-3xl bg-gradient-to-br from-indigo-950/80 via-purple-950/50 to-[#121829] border-2 border-[#8b5cf6]/70 shadow-[0_0_20px_rgba(139,92,246,0.25)] space-y-3">
              <div className="flex items-center justify-between gap-2">
              ...
              <div>
                <h3 className="text-sm font-black text-white flex items-center gap-1.5">
                  <span>⚡ Seguir Padrão da Trilha</span>
                </h3>
                <p className="text-xs text-slate-300 mt-0.5">
                  Estude o conteúdo padrão programado automaticamente para a sua série escolar:
                </p>
              </div>

              {/* Preview Box */}
              <div className="p-3 rounded-2xl bg-[#0b0f19]/80 border border-purple-500/30 space-y-1">
                <div className="text-xs font-black text-white flex items-center gap-1.5">
                  <span className="text-base">📌</span>
                  <span className="text-purple-200">{defaultTopic.title}</span>
                </div>
              ...
              <button
                onClick={handleFollowDefaultTopic}
                className="w-full py-3.5 px-5 rounded-2xl bg-gradient-to-r from-emerald-600 via-teal-600 to-indigo-600 hover:from-emerald-500 hover:to-indigo-500 text-white font-black text-xs transition shadow-lg shadow-emerald-900/30 flex items-center justify-center gap-2 cursor-pointer active:scale-98"
              >
                <span>Seguir Conteúdo Padrão 🚀</span>
                <ChevronRight className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="flex items-center gap-2 pt-1">
              ...
              <span className="text-[10px] font-black uppercase tracking-wider text-slate-400">
                Ou Escolha o Conteúdo que Deseja do Ano
              </span>
              <div className="h-px flex-1 bg-[#1e293b]" />
            </div>
            ...
            <div className="space-y-2.5 pb-2">
              {availableTopics.map((topic, index) => {
                const isDefault = topic.isDefault;
                return (
                  <div
                    key={topic.id}
                    className={`p-3.5 rounded-2xl border transition-all space-y-2.5 ${
                      isDefault
                        ? 'bg-[#151c33] border-purple-500/50'
                        : 'bg-[#121829] border-[#1e293b] hover:border-slate-600'
                    }`}
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          {isDefault && (
                            <span className="px-2 py-0.5 rounded-full bg-purple-500/20 text-purple-300 text-[9px] font-black">
                              Padrão
                            </span>
                          )}
                        </div>
                    ...
                    <div className="flex items-center justify-between pt-1 border-t border-[#1e293b]/60">
                      <span className="text-[10px] font-semibold text-slate-400">
                        {topic.questionsCount} questões • Teoria com Voz IA
                      </span>
                      <button
                        onClick={() => handleSelectLesson(topic.lesson, true)}
                        className="px-3.5 py-1.5 rounded-xl bg-purple-600/20 hover:bg-purple-600/30 border border-purple-500/40 text-purple-200 hover:text-white text-xs font-bold transition active:scale-95 flex items-center gap-1 cursor-pointer"
                      >
                        <span>Aprender</span>
          ...
          <div className="pt-2">
            <button
              ...
              className="w-full py-3 px-4 rounded-xl bg-[#121829] hover:bg-[#161f38] text-slate-400 hover:text-slate-200 border border-[#1e293b] font-bold text-xs transition text-center cursor-pointer"
            >
      ...
      {!isLoading && currentStep === 'lesson_intro' && activeLesson && (
        <div className="flex-1 flex flex-col justify-between space-y-4 overflow-y-auto pr-1">
          <div className="space-y-4">
            ...
            <div className="p-3 rounded-2xl bg-[#121829] border border-[#1e293b] flex items-center justify-between gap-2 shadow-sm">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <h4 className="text-xs font-black text-purple-200 truncate">
                    {activeLesson.title}
                  </h4>
                </div>
              </div>
              ...
              <button
                ...
                className="shrink-0 px-2.5 py-1 rounded-xl bg-[#161f38] hover:bg-purple-950/50 border border-[#1e293b] hover:border-purple-500/50 text-slate-300 hover:text-purple-200 text-[11px] font-bold transition flex items-center gap-1 cursor-pointer"
              >
